...
const UIEngine = {
    ...
    saveSiteInfo: function() {
        ...
        fetch('/api/sites/' + encodeURIComponent(siteId) + '/update-site', {
            ...
            // No dev server — download the updated JSON so user can drop it into data/sites/
            status.textContent = 'No server — downloading updated JSON instead…';
            btn.disabled = false;
            var existing = JSON.parse(JSON.stringify(window.__SITE_DEFAULTS__ || {}));
            ...
            editable.forEach(function(k) { if (payload[k] !== undefined) existing[k] = payload[k]; });
            var out = { project: 'ProjectBook-Planner', site: existing, saved: (window.__SAVED_STATE__ || {}) };
            var blob = new Blob([JSON.stringify(out, null, 4)], { type: 'application/json' });
            var a = document.createElement('a');
            a.href = URL.createObjectURL(blob);
            a.download = siteId.toLowerCase().replace(/[^a-z0-9]/g, '_') + '_site.json';
            a.click();
            URL.revokeObjectURL(a.href);
            status.textContent = 'Downloaded — place in data/sites/ and rebuild.';
        });
    }
};
